...
public class TheWonderlandModBiomes {
	public static final DeferredRegister<Biome> REGISTRY = DeferredRegister.create(ForgeRegistries.BIOMES, TheWonderlandMod.MODID);
	public static final RegistryObject<Biome> CANDYLAND = REGISTRY.register("candyland", () -> CandylandBiome.createBiome());
			...
			for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : worldGenSettings.dimensions().entrySet()) {
				...
				if (dimensionType == dimensionTypeRegistry.getOrThrow(DimensionType.OVERWORLD_LOCATION)) {
					...
					if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
						...
						parameters.add(new Pair<>(CandylandBiome.PARAMETER_POINT,
								biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, CANDYLAND.getId()))));
						parameters.add(new Pair<>(GhostBiomeBiome.PARAMETER_POINT,
								biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, GHOST_BIOME.getId()))));
						parameters.add(new Pair<>(DemonicAreaBiome.PARAMETER_POINT,
					...
					if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
						...
						if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
							...
							surfaceRules.add(1,
									preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, CANDYLAND.getId()),
											TheWonderlandModBlocks.CANDY_GRASS_BLOCK.get().defaultBlockState(),
											TheWonderlandModBlocks.CANDYDIRT.get().defaultBlockState(),
											TheWonderlandModBlocks.CANDYDIRT.get().defaultBlockState()));
							surfaceRules.add(1,
									preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, GHOST_BIOME.getId()),
											TheWonderlandModBlocks.SOUL_GRASS.get().defaultBlockState(),
											TheWonderlandModBlocks.SOULDIRTT.get().defaultBlockState(),
											TheWonderlandModBlocks.SOULDIRTT.get().defaultBlockState()));
							surfaceRules.add(1,
									preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, DEMONIC_AREA.getId()),
											Blocks.CRIMSON_NYLIUM.defaultBlockState(), Blocks.NETHER_WART_BLOCK.defaultBlockState(),
